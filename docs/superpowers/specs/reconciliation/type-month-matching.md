...
5. 多对多合计金额相等只允许人工确认，不进入自动 `MATCHED`，且不得跨当前账单月份混配。
...
6. 先处理唯一可自动配对部分
7. 再处理同金额多笔待确认部分
8. 再识别同成员 + 同险种 + 当前账单月份 + 同费款所属期下合计金额相等但单笔金额不一致的多对多待确认部分
9. 最后处理未闭合残留记录
10. 回填系统侧和台账侧结果字段
11. 生成本轮统计结果
...
3. 再识别同成员 + 同险种 + 当前账单月份 + 同费款所属期下合计金额相等的多对多 `PENDING` 组
...
- 回填规则与自动匹配一致
- 若为多对多手动核对，则生成同一个 `manualMatchGroupId`
- 多对多记录的跨引用展示为“多对多匹配组”，不要求每条系统记录绑定唯一台账记录
- 取消任一多对多匹配组内记录时，整组系统侧和台账侧记录一起恢复为待确认状态
...
| S3A | 多对多合计金额相等待确认 | `主体A + 规则A + 2026-04` | `王五/养老/补缴/费款所属期2026-04/100.00`、`王五/养老/补缴/费款所属期2026-04/200.00` | `王五/养老/应缴月份2026-04/费款所属期2026-04/150.00`、`王五/养老/应缴月份2026-04/费款所属期2026-04/150.00` | 开始对账后人工选择 2+2 确认 | 4 条记录进入 `PENDING`；确认时系统合计 300.00 = 台账合计 300.00，全部转 `MATCHED` 并绑定同一 `manualMatchGroupId` |
...
- 多对多合计场景下，用户可同时勾选多笔系统侧和多笔台账侧
- 多对多确认前必须校验同成员、同险种、当前账单月份、同费款所属期、系统合计金额 = 台账合计金额
- 多对多合计不相等时，显示错误提示并禁用确认
- 确认后：系统侧回填 `payableMonth`，台账侧回填 `feeTypeInferred`，两侧状态变为 MATCHED；多对多记录绑定同一个 `manualMatchGroupId`
