...
在员工异动操作页面（增员/减员/调基/补缴）中，"基本信息"和"参保信息"下方、"险种信息"上方，新增**"采集字段"**卡片：
...
| `file` | 文件上传区域（拖拽/点击） | 文件格式+大小校验 |
| `photo` | 照片上传（支持预览） | 图片格式+尺寸校验 |
| `select` | `<select>` 下拉选择 | 值在选项列表中 |

### 6.4 AUTO 字段展示

- 按 `auto_source_path` 从员工档案 JSON 中读取值
- 使用 `<input readonly>` 展示，样式为灰色背景
- 字段下方标注来源路径（如 `来源：employee.contract.startDate`）
- 区块顶部展示"已从员工档案获取"绿色徽章
...
- 增员操作：8 个采集字段（5 个 AUTO + 3 个 MANUAL）
- 操作类型切换后字段集变化
- 文件上传模拟、必填校验、正则校验
- 提交动画与 Toast 提示
