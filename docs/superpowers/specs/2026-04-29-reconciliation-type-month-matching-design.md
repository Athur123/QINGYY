...
| `.batch-row` | 批次行容器，粗体背景，hover 高亮 |
| `.batch-expand__icon` | 展开/收起图标（▸/▼），`.is-expanded` 时旋转 90 度 |
| `.batch-row__name` | 批次文件名（粗体 13px） |
| `.batch-row__meta` | 批次元信息（导入时间 + 规则数，11px 灰色） |
| `.sub-row` | 规则子行容器 |
...
| `ledgerBatches` | 台账批次数组，每次导入生成一个批次 {batchId, fileName, importDate, ruleNames, ...} |
| `batchCounter` | 批次计数器，用于生成唯一 batchId |
| `expandedBatches` | Set，当前展开的批次 ID 集合 |
...
22. **汇总页改为台账批次维度**：表格行从"一条规则一行"改为"一个台账批次一行"。每次导入台账生成一个批次（`ledgerBatches`），批次行可展开显示该批次包含的各规则明细。表头变为：checkbox → 批次名称/导入时间 → 规则数 → 记录数 → 台账侧总计 → 系统侧已核对 → 系统侧差异 → 台账侧已核对 → 台账侧差异 → 台账侧待确认 → 操作。子行显示规则名称、参保主体、月份及各统计数字。信息栏新增"X 个批次"计数。
...
──────────────────────────────────────────────────────────────────────────────────────────────────┤
│  共 6 条规则，1 个批次，已导入 6 条，已归档 0 条                                                    │
├──────┬─────────────────────────────┬──────┬──────┬──────────┬──────────┬──────────┬──────────┬──────────┬──────────┤
│     │ 批次名称 / 导入时间          │ 规则数│ 记录数│ 台账侧   │ 系统侧   │ 系统侧   │ 台账侧   │ 台账侧   │ 台账侧   │ 操作      │
│      │                             │      │      │ 总计     │ 已核对   │ 差异     │ 已核对   │ 差异     │ 待确认   │          │
├──────┼─────────────────────────────┼──────┼──────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────────┤
│ ☑   │ 台账导入_20260506.xlsx     │  6   │  20  │ 20笔     │ ¥6,475.43│ ¥1,100.00│ ¥6,475.18│ ¥1,210.23│ ¥3,427.62│ [对账]   │
│      │ 2026-05-06 18:29 · 6条      │      │      │ ¥7,685.41│          │          │          │          │          │          │
├──────┴─────────────────────────────┴──────┴──────┴──────────┴──────────┴──────────┴──────────┴──────────┴──────────┤
...
| 批次名称 / 导入时间 | 批次文件名 + 导入时间 + 规则数，点击可展开/收起规则子行 |
| 规则数 | 该批次包含的规则数量 |
| 记录数 | 该批次包含的台账记录总数 |
| 台账侧总计 | 两行：总笔数 / 总金额（单元格内 flex column 展示），批次行 = 子行聚合 |
| 系统侧已核对 | 两行：笔数 / 已匹配金额合计，批次行 = 子行聚合，有值时绿色 |
...
| 操作 | 批次行：`[对账]` 链接，一键对账该批次下所有已导入规则。子行：`[详情]` 跳转到明细页 |
...
**批次展开**: 点击批次行（或展开图标 ▸/▼）切换子行显示/隐藏。新导入的批次自动展开。

**样式约定**:
- 批次行：粗体背景（var(--qy-bg-tertiary)），hover 时高亮
...
// 台账批次（每次导入生成一个批次）
let ledgerBatches = [];         // [{batchId, fileName, importDate, ruleNames: [], totalRecords, totalAmount, reconciledCount, diffCount}]
let batchCounter = 0;           // 批次计数器
let expandedBatches = new Set();// 当前展开的批次ID集合
...
// 台账批次（每次导入生成一个批次）
// {
//     batchId: 'BATCH-001',          // 批次ID，自增
//     fileName: '台账导入_20260506.xlsx',  // 文件名
//     importDate: '2026-05-06 18:29',  // 导入时间
//     ruleNames: ['社保规则A', ...],   // 该批次包含的规则名称列表
//     totalRecords: 20,               // 台账记录总数
//     totalAmount: 7685.41,           // 台账总金额
//     reconciledCount: 6,             // 已完成对账的规则数
//     diffCount: 3,                   // 有差异的规则数
// }
...
| 元素 | 变更 |
|------|------|
| 表头 | 新增 checkbox 列、批次名称/导入时间列、规则数列、记录数列；列顺序：checkbox → 批次名称/导入时间 → 规则数 → 记录数 → 台账侧总计 → 系统侧已核对 → 系统侧差异 → 台账侧已核对 → 台账侧差异 → 台账侧待确认 → 操作 |
| 批次行 | 粗体背景（var(--qy-bg-tertiary)），显示批次文件名+导入时间+规则数，点击可展开/收起子行 |
...
| 导入对话框 | 上传区（拖拽/点击）+ 预览表格 + 确认按钮，导入时生成批次记录 |
| Loading 覆盖层 | 批量对账执行期间显示"正在批量对账..." |
| 信息栏 | 显示"共 N 条规则，X 个批次，已导入 M 条，已归档 K 条" |
...
| `toggleBatchExpand(batchId)` | 展开/收起指定批次的规则子行 |
| `toggleBatchCheckbox(checkbox)` | 按批次选择：选中批次内所有规则 |
| `batchReconcileByBatch(batchId)` | 对单个批次执行批量对账（选中该批次下所有已导入规则后调用 batchReconcile） |
...
1. 打开汇总页 → 页面显示 0 个批次，提示"暂无批次数据，请先导入台账"
2. 点击"导入台账" → 上传模拟文件 → 预览表格展示按规则拆分结果 → 确认导入
3. 信息栏更新"共 6 条规则，1 个批次，已导入 6 条"，显示一个批次行（自动展开）
4. 批次行显示：批次名称（台账导入_xxx.xlsx）、导入时间、8条规则、20条记录、台账侧总计金额
5. 展开后的子行显示各规则明细（规则名称、参保主体、月份、各统计数字，初始为"—"）
6. 勾选批次 checkbox → 选中该批次下所有已导入规则 → "批量对账"按钮变为可点击
7. 点击"对账"（批次行操作列）或"批量对账" → loading 覆盖层 → 完成后批次行显示聚合金额，子行显示各规则对账结果
8. 未导入的规则 checkbox 不可勾选，无法参与对账
9. 筛选月份/地区后，导入和对账仍正常工作
10. 勾选对账无差异的规则 → "批量归档"按钮变为可点击
11. 点击"批量归档" → 批次内规则行变灰，信息栏显示归档计数
12. 已归档规则不参与全选，不可再次归档
13. 有差异的子行显示浅黄色背景（#FFFBEB），无差异的已对账行显示浅绿色背景（#F0FDF4）
14. 批次行可点击展开/收起，展示/隐藏规则子行
15. 批次行的台账侧总计 = 子行台账金额合计，系统侧已核对/差异 = 子行聚合
