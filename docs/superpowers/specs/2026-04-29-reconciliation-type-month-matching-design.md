...
| `forceMatch(recordIds)` / `showForceMatchConfirm(recordIds)` / `closeForceMatchDialog()` / `executeForceMatch()` / `systemBatchForceMatch()` | 明细页系统侧强制核对（单行/批量），仅系统侧 DIFF/UNMATCHED 记录可用，台账侧不允许强制核对 |
...
31. **系统侧操作列增加强制核对按钮**：DIFF/UNMATCHED 状态的行操作列从"详情"改为"强制核对"，调用 `showForceMatchConfirm(['${r.id}'])`。
32. **台账侧不允许强制核对**：台账侧 DIFF/UNMATCHED 行操作列保持"详情"，不显示"强制核对"。台账侧账单必须有对应的系统侧账单明细进行匹配核对。
...
├──────┬──────────────┬──────┬──────┬─────────── 系统侧明细 ───────────┬─────────── 台账侧明细 ───────────────────────┬──────┤
│      │ 参保主体      │参保规则│账单月份│ 已核对   │ 差异     │ 已归档   │ 总计     │ 已核对   │ 差异     │ 待确认   │ 操作  │
├──────┼──────────────┼──────┼──────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────────┼──────┤
│ ☑   │ 某科技有限公司  │ 3条  │2026-04│ ¥3,200.00│ ¥500.00  │ ¥3,200.00│ 10笔     │ ¥3,200.00│ ¥500.00  │ ¥1,500.00│[详情] │
│     │ 重庆           │      │       │          │          │          │ ¥4,500.00│          │          │          │[对账] │
├──────┴──────────────┴──────┴──────┴──────────┴──────────┴──────────┴──────────┴──────────┴──────────┴──────────┴──────┤
│      │ 社保规则A  某科技有限公司   2026-04│ 3笔     │ —       │ 3笔     │ 4笔     │ 2笔     │ 1笔     │ 1笔     │[详情]│
│      │ 社保规则B  某科技有限公司   2026-04│ 1笔     │ —       │ —       │ 1笔     │ 1笔     │ —       │ —       │[详情]│
│      │ ...                                                                                                         │
└──────┴──────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
| 系统侧已归档 | 两行：已归档笔数 / 已归档金额合计；无已归档显示 "—"，有值时绿色。批量归档时 systemMatched → systemArchived |
...
**表头**: 采用二级结构，一级「系统侧明细」(已核对/差异/已归档) 和「台账侧明细」(总计/已核对/差异/待确认)，两组之间用 2px 边框分隔。
...
- 子行颜色编码：已对账无差异 → 浅绿色（`#F0FDF4`），已对账有差异 → 浅黄色（`#FFFBEB`），已归档 → 灰色（`#F1F5F9`，opacity 0.7），已付款 → 蓝色（待定）
...
当前原型中 `doArchive()` 将所有 MATCHED 记录一次性标记为 `archived=true`，无批次概念。实际业务中同一账单月份可能经过多轮对账——系统侧新增员工/险种产生新费用，或台账侧追加导入新数据——每次都需要对新增记录进行核对、归档，形成多个归档批次。

### 约束确认

| 项目 | 确认值 |
|------|--------|
| 新增来源 | 系统侧和台账侧都可能产生新数据 |
| 归档粒度 | 全部"已核对且未归档"记录自动归为一个新批次 |
| 批次历史 | 需要查看历史批次及其包含的记录 |
...
const archiveBatches = [
  {
    id: 'B001',                    // 批次ID
    label: '第1批',                // 显示标签
    billingMonth: '2026-04',       // 账单月份
    createdAt: '2026-04-28 10:30:00',  // 归档时间
    recordCount: 9,                // 本批记录数
    totalAmount: 5800.00,          // 本批金额合计
  },
];
```
...
系统侧和台账侧记录各新增 `archiveBatchId` 字段：
...
点击「归档结果」
  │
  ├─ 前置检查：matchingResults.executed 是否为 true
  │
  ├─ 筛选待归档：所有 matchStatus=MATCHED 且 archived=false 的记录
  │
  ├─ 空批次检查：无待归档记录 → toast "无待归档记录"
  │
  ├─ DIFF/PENDING 检查：
  │   系统侧是否存在未归档的 PENDING/DIFF？
  │   ├─ 有 → 弹窗确认"还有 N 条待确认/M 条差异未处理，是否继续？"
  │   │       仅对 MATCHED 记录归档（不影响 PENDING/DIFF）
  │   └─ 无 → 直接归档
  │
  ├─ 创建批次：
  │   ├─ id: 'B' + (archiveBatches.length + 1).toString().padStart(3, '0')
  │   ├─ label: '第' + archiveBatches.length + 1 + '批'
  │   ├─ billingMonth: currentBillingMonth
  │   ├─ createdAt: new Date().toLocaleString('zh-CN')
  │   ├─ recordCount: 本次归档的系统侧记录数
  │   └─ totalAmount: 本次归档的系统侧金额合计
  │
  ├─ 标记记录：
  │   系统侧 MATCHED+unarchived → archived=true, archivedAt=now, archiveBatchId=新批次ID
  │   台账侧 MATCHED+unarchived → archived=true, archivedAt=now, archiveBatchId=新批次ID
  │
  └─ Toast: "归档完成：第N批，共X条，¥XX,XXX.XX"
```

### 多次归档场景

```
初始状态：billingMonth=2026-04，系统侧10条，台账侧10条

回合1：
  ├─ 导入台账 → 执行对账 → 全部 MATCHED
  ├─ 归档 → 创建「第1批」(10条, ¥5,800)
  └─ 10条系统侧 + 10条台账侧标记为已归档

回合2（两周后）：
  ├─ 系统侧新增2条（新员工补缴），台账侧追加导入2条
  ├─ 表格中：10条灰色已归档·第1批 + 2条白色未归档 + 2条白色未归档台账
  ├─ 执行对账 → 新增2条进入 MATCHED
  ├─ 归档 → 创建「第2批」(2条, ¥900)
  └─ 批次历史：[第2批|2条|¥900|05-03] [第1批|10条|¥5800|04-28]
```
