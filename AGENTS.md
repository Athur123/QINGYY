...
6. 修改 `AGENTS.md` 或 `CLAUDE.md` 时，必须同步另一份文件中的共享项目事实，并检查两份文件是否仍然一致。

## 双 Agent 支持

本仓库同时支持 **Codex** 与 **Claude Code**。
...
主要内容：`prototype/`（HTML 原型）、`styles/`（设计系统 CSS/SCSS）、`docs/`（文档）、`.agents/skills/`（Codex 技能）、`scripts/`（工具脚本）。
...
技能位于 `.agents/skills/`，用于自动化操作线上青阳云 HRO 系统。
...
凭据来源：`.codex/config.toml`（不提交，模板见 `.codex/config.example.toml`）。
