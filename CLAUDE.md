...
本仓库从项目根目录同时支持 **Claude Code** 与 **Codex**。
...
- 安全配置模板放在 `.claude/settings.local.example.json` 和 `.codex/config.example.toml`。
...
4. `.claude/skills/`：Claude Code 使用的青阳云自动化技能。
5. `.agents/skills/`：Codex 使用的青阳云自动化技能。
...
Claude Code 项目技能位于 `.claude/skills/`，用于自动化操作线上青阳云 HRO 系统。Codex 对应技能保留在 `.agents/skills/`，用于跨工具兼容。
...
- 凭据来源：`.claude/settings.local.json` 中的本地环境变量。
...
真实值只放在 `.claude/settings.local.json`，不要提交。安全模板见 `.claude/settings.local.example.json`。
...
Codex 读取根目录的 `AGENTS.md`。当更新项目事实、目录约定、设计系统版本、原型入口或安全配置说明时，同步检查 `CLAUDE.md` 与 `AGENTS.md`，避免两个 Agent 入口给出不同指令。

Codex 相关内容通常位于：

- 技能目录：`.agents/skills/`
- 本地配置：`.codex/config.toml`
- 安全模板：`.codex/config.example.toml`

## 开发与验证流程

本项目建议采用 `brainstorming → spec → plan → implement` 的节奏处理较大需求：

1. `brainstorming`：先明确业务目标、约束、边界和验收标准。
2. `spec`：把确认后的设计写入 `docs/superpowers/specs/<module>/<name>.md`。
3. `plan`：把 spec 拆成可执行任务，写入 `docs/superpowers/plans/<module>/<name>.md`。
4. `implement`：按计划修改原型、样式或文档，并进行浏览器验证。

小范围文档或样式修正可以直接执行，但仍要先阅读相关上下文。
...
涉及视觉、交互或布局变化时，使用 Playwright 或浏览器自动化打开本地页面验证。
...
7. 修改 `CLAUDE.md` 或 `AGENTS.md` 时，尽量同步另一份文件中的共享项目事实。
